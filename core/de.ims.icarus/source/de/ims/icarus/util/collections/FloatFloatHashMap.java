...
public class FloatFloatHashMap {
	...
	private static final float DEFAULT_RESULT = -1;
	...
	private static class Entry {
		 ...
		 */
		protected Entry(int hash, float key, float value, Entry next) {
			this.hash = hash;
			this.key = key;
			this.value = value;
			this.next = next;
	 ...
	 */
	public FloatFloatHashMap(int initialCapacity, float loadFactor) {
	 ...
	 * 
	 * The value can be retrieved by calling the {@link #get(float)} method with a

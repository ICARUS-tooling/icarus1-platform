...
public class NGramSearch extends AbstractParallelSearch implements
	 ...
	 * @param target
	 */
	public NGramSearch(NGramSearchFactory factory, SearchQuery query,
			Options options, Object target) {
		super(factory, query, options, target);

		useNumberWildcard = getParameters().getBoolean(USE_NUMBER_WILDCARD,
				DEFAULT_USE_NUMBER_WILDCARD);
		useFringe = getParameters().getBoolean(USE_FRINGE_HEURISTIC,
				DEFAULT_USE_FRINGE_HEURISTIC);
		fringeSize = getParameters().getInteger(FRINGE_SIZE,
				DEFAULT_FRINGE_SIZE);
		ngramResultLimit = getParameters().getInteger(NGRAM_RESULT_LIMIT,
				DEFAULT_NGRAM_RESULT_LIMIT);
		sentenceLimit = getParameters().getInteger(SENTENCE_LIMIT,
				DEFAULT_SENTENCE_LIMIT);
		gramsGreaterX = getParameters().getInteger(GRAMS_GREATERX,
				DEFAULT_GRAMS_GREATERX);
		createXML = getParameters().getBoolean(CREATE_XML_OUTPUT,
				DEFAULT_CREATE_XML_OUTPUT);
	}

	public NGramSearch(NGramSearchFactoryDependency factory, SearchQuery query,
			Options options, Object target) {
		super(factory, query, options, target);
	...
	protected SearchResult createResult() {
		...
		if (annotator != null && result instanceof AbstractSearchResult) {
			AnnotationBuffer annotationBuffer = new AnnotationBuffer(result,
					annotator, ANNOTATION_BUFFER_SIZE);
			((AbstractSearchResult) result)
					.setAnnotationBuffer(annotationBuffer);
		}
	...
	protected SentenceDataList createSource(Object target) {
		...
		if (entryType == null
				|| !ContentTypeRegistry.isCompatible(requiredType, entryType))
			throw new IllegalArgumentException(
					"Target is not a container holding dependency data items"); //$NON-NLS-1$
	...
	protected class NGramWorkerDependency extends Worker {
		...
		protected void init() {
			...
			// TO DO test stuff only pos filtering
			...
			// List<String> tmpKey = new
			// ArrayList<String>(ngramsResultMap.keySet());
			...
			// ArrayList<DependencyItemInNuclei> value =
			// ngramsResultMap.get(key);
			...
			((ErrorMiningMatcherDependency) matcher)
					.setResultNGramsDependency(ngramsResultMap);
...
//		/**
//		 * @see de.ims.icarus.search_tools.standard.AbstractParallelSearch.Worker#isCancelled()
//		 */
//		@Override
...
//		}
		...
		protected void init() {
			...
			ngrams = new NGrams(ngramOptions, createQueryList(), NGramSearch.this);
			...
			// //ggf direkt in matcher erzeugen lassen
			// matcher.setSentenceList(helferList);
			// ((NGramResultMatcherPoS)
			// matcher).setResultNGrams(ngramsResultMap);
		...
		protected void process() {
			...
			if (ngramsResultMap.size() > 0) {
				...
				// // equals; note all indices saved within the list are results
				// and have
	...
	/**
	 * we only want every sentence at most once in our results, therefore we
	 * have to check if for a given key we already created a hit. this is done
	 * by simple string contains matching. When a ngram contains more than one
	 * key this is already represented by two nucleis within the ngram.
	 ...
	 * @return
	 */
	private boolean disjunctNuclei(MappedNGramResult mapping, String newKey,
			List<MappedNGramResult> helferList) {
		...
		// System.out.println(mapping.getCoverStart() + " " +
		// mapping.getCoverEnd());
	...
	private static class NGramSearchResult extends DefaultSearchResult0D {
		...
		public ContentType getContentType() {
			return ContentTypeRegistry.getInstance().getTypeForClass(
					NGramData.class);
		}

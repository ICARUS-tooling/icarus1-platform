...
import de.ims.icarus.util.transfer.ConsumerRegistry;
...
public final class IcarusCorePlugin extends Plugin {
	...
	private void registerUIHelpers() {
		for(Extension extension : extensions("UIHelper")) { //$NON-NLS-1$
			try {
	...
	private void registerContentTypes() {
		for(Extension extension : extensions("ContentType")) { //$NON-NLS-1$
			try {
	...
	private void registerErrorFormatters() {
		for(Extension extension : extensions("ErrorFormatter")) { //$NON-NLS-1$
	...
	private void registerSerializables() {
		for(Extension extension : extensions("Serializable")) { //$NON-NLS-1$
	...
	private void registerConsumers() {
		//FIXME
		for(Extension extension : extensions("Consumer")) { //$NON-NLS-1$
			ConsumerRegistry.getInstance().register(new ConsumerProxy(extension));
		}
	}

	private Collection<Extension> extensions(String extensionPointId) {
		Collection<Extension> extensions = getDescriptor().getExtensionPoint(extensionPointId).getConnectedExtensions();
		if(extensions==null) {
